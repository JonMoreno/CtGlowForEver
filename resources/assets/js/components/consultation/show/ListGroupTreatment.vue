...
<template>
    <div class="py-3 px-2">
        <h5 class="font-weight-bold">Treatments applied</h5>
        <div class="list-group">
            <a v-if="info.length == 0"
                class="list-group-item list-group-item-action text-center px-2">
                <p class="text-muted font-weight-bold">
                     <i class="fa fa-info-circle fa-lg text-primary" aria-hidden="true"></i>Treatments Were Not Applied.
                </p>
            </a>
            <a v-else v-for="(purchase, index) in info" :key="purchase.id"
                ...
                <div class="d-flex w-100 justify-content-between align-items-center">
                    <div class="text-capitalize">
                        ...
                        {{ purchase.treatment.name }}
                    </div>
                    <div>
                        Sold Price:
                        <span class="badge badge-success py-2">
                            ${{soldAtPrice(index, purchase.treatment_id)}}
                        </span>
                    </div>
                </div>

                <p v-if="hasDiscount(purchase.treatment_id)" class="bg-light-danger rounded my-2 py-1 pl-1">
                    <strong>Discount Applied:</strong> <br/>

                    <span class="pl-2">{{ discountName(purchase.treatment_id) }}</span> <br/>
                    ...
                    <span class="pl-2">Original Price:<del>{{ (purchase.sold_price) }}</del>  
                        ...
                        <strong class="text-danger">
                            {{discountValue(purchase.treatment_id) }}
                        </strong>
    ...
    //discount applicable operations.
        discountName: function(ref) {
            ...
            for(let i = 0; i < size; i++){
                if(discounts[i].service == 'treatment' && discounts[i].service_id == ref){
